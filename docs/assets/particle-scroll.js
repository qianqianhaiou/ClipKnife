/*
 * Particle Scroll from Canvas UI
 * https://github.com/DavidHDev/canvas-ui
 * Copyright (c) 2026 David Haz
 * Licensed under MIT + Commons Clause. See canvas-ui-LICENSE.md.
 */
// src/lib/ParticleScroll/ParticleScrollVanilla.ts
var DEFAULTS = {
  point: 0.68,
  band: 420,
  density: 2,
...
function createParticleScroll(elements, options = {}) {
  ...
  function updateRows(dt, density, winStart, winLen) {
    ...
    if (rowWindow.length !== winLen)
      rowWindow = new Float32Array(winLen);
    rowsAnimating = false;
    let minP = 1;
  ...
  function frame(now) {
    ...
    if (reducedMotion || Math.abs(lag) < 0.1)
      lag = 0;
    if (!introDone) {
      if (reducedMotion || !htmlInCanvas)
        introDone = true;
      else if (introReady) {
        introWait += delta;
        if (introWait >= 1)
    ...
    const k = reducedMotion || tau <= 0 ? 1 : 1 - Math.exp(-delta / Math.max(tau, 0.0001));
    scrollSmooth += (scrollTop - scrollSmooth) * k;
    if (Math.abs(scrollTop - scrollSmooth) < 0.5)
