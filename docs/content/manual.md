...
打开“创作助手设置 → 模型配置”。左侧的“服务端供应商”由素刀服务提供，并按服务端设置的顺序固定展示；“本地配置”保存你自行添加的供应商，以及服务端已经移除但你仍在使用的旧配置。

选择一个服务端供应商后，按行完成配置：

1. 确认供应商名称。固定供应商由服务端维护，不能在客户端修改。
2. 确认 API Base URL。固定供应商由服务端维护，不能在客户端修改。
3. 从模型 ID 下拉选项中选择。固定供应商不允许自行输入列表之外的模型 ID。
4. 查看上下文长度。固定供应商的上下文长度由服务端按模型设置，切换模型时会自动更新，不能在客户端修改。
5. 填写 API Key。已经安全保存过的密钥不需要重复输入；API Key 只保存在当前设备，不会上传到素刀服务端。
6. 选择思考强度，再点击“测试连接”。测试成功后可将该配置设为默认。
7. 如果页面显示“配置说明”，请阅读 warning 提示卡中的配置简介；供应商提供完整教程时，可继续点击卡片内的链接查看。服务端既没有提供简介也没有提供链接时，不显示这一行。

没有想用的固定供应商时，点击“添加自定义供应商”，填写名称、API Base URL、模型 ID、上下文长度和 API Key。自定义供应商可以自行输入模型 ID 和上下文长度，但仅支持 OpenAI Compatible 协议；上下文长度默认是 256k（262,144 tokens）。删除自定义供应商前，素刀会再次提示确认；删除后历史会话内容仍会保留，但依赖该供应商的历史会话无法继续对话。暂时离线时，素刀会优先使用同一远程服务最近一次成功获取的供应商目录；首次离线且没有缓存时，仍可继续使用已有本地配置或添加自定义供应商。

服务端修改固定供应商的名称、Base URL 或所选模型的上下文长度后，关联配置会自动同步这些公开信息，但不会覆盖你的模型选择、API Key、思考强度和默认选择。服务端移除供应商后，已经配置的项目仍保留在“本地配置”中。
