...
创作助手也可以调用这个工具。它生成的 WAV 文件会直接保存在当前会话工作区，不会进入工具箱输出目录或“最近生成”列表。当前会话已经选择草稿时，创作助手默认把生成结果加入从 0 秒开始的独立“AI配音”音频轨道；如果你明确要求只生成文件，则不会修改草稿。即使导入草稿失败，已经生成的 WAV 文件也会保留。
