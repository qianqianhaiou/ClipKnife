...
4. 填写 API Key。已经安全保存过的密钥不需要重复输入；API Key 只保存在当前设备，不会上传到素刀服务端。
5. 选择思考强度，再点击“测试连接”。测试成功后可将该配置设为默认。
6. 如果页面显示“配置说明”，请阅读 warning 提示卡中的配置简介；供应商提供完整教程时，可继续点击卡片内的链接查看。服务端既没有提供简介也没有提供链接时，不显示这一行。

没有想用的固定供应商时，点击“添加自定义供应商”，填写名称、API Base URL、模型 ID 和 API Key。自定义供应商可以自行输入模型 ID，但仅支持 OpenAI Compatible 协议。暂时离线时，素刀会优先使用同一远程服务最近一次成功获取的供应商目录；首次离线且没有缓存时，仍可继续使用已有本地配置或添加自定义供应商。

服务端修改固定供应商的名称或 Base URL 后，关联配置会自动同步这些公开信息，但不会覆盖你的模型选择、API Key、思考强度和默认选择。服务端移除供应商后，已经配置的项目仍保留在“本地配置”中。
