...
AI 与媒体组件设置会展示 TransNetV2、ffmpeg/ffprobe、imsearch 和语音转写组件等本地能力。它们的状态会影响分镜、抽帧、视觉检索、相似检索和语音检索是否可用。组件管理会同时展示 imsearch 与 TransNetV2 的当前版本、运行版本和最新版本。

如果 imsearch 或 TransNetV2 需要更新，组件管理会给出更新提示和软件网盘入口。请从软件网盘下载对应的组件压缩包，再回到组件管理选择压缩包并确认替换；应用不会静默自动安装组件包。
