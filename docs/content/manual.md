...
ClipKnife（素刀）是一款本地素材管理、智能检索与辅助创作工具。你可以把分散在文件夹、盘符和外接硬盘里的图片、视频、音频素材加入素材库，通过自然语言快速找回内容，再使用创作助手整理素材、生成文本或配音、创建和编辑剪映草稿。工具箱还提供按需安装的文本转语音、图片清晰化和视频增强能力。
...
工具箱以卡片列表展示素刀已经适配的本地创作工具，当前包括“文本转语音”“图片清晰化”和“视频增强”。首次使用某个工具前，先在左侧导航打开“工具箱”，在对应卡片上点击“安装工具”；未安装时不能进入工具详情。安装完成后，卡片会显示“进入工具”，有新版本时同一位置也会显示更新入口。下载、校验和解压过程会显示进度，也可以在任务中心查看。
...
图片和视频工具都不会覆盖输入文件。处理结果保存在素刀工具箱目录；取消或失败时会清理未完成结果，视频组合处理还会清理中间文件。两项增强都依赖支持 Vulkan 的显卡，视频处理尤其可能消耗较长时间、较多显存和磁盘空间。
...
ClipKnife 是本地优先产品。素材扫描、索引、搜索、分镜、转写，以及工具箱中的文本转语音、图片清晰化和视频增强都在本机处理。
...
- 创作助手使用你配置的模型供应商时，会按任务需要发送对话内容、系统上下文、工具结果、已有视觉描述，以及你明确选择的本地文件路径。API Key 只保存在当前设备。
...
- 工具箱安装和更新需要连接素刀远程服务；图片和视频增强需要支持 Vulkan 的显卡。
