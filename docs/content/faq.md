...
标签已保存在本地，但本地搜索索引还没有处理完最新更新，因此当前结果可能暂未包含最新标签。这不是云同步，也不会上传原始素材。

点击搜索提示旁的“查看标签同步任务”，可以查看等待状态、已同步数量和失败原因。服务繁忙或写入失败时会自动延后重试；连续失败后，请检查本地 AI 组件并在任务中心使用“重试失败”。如果队列已暂停，需要先点击“全部开始”；如果索引尚未就绪，需要等待索引建立。应用重启后会恢复中断的同步任务，清理任务记录不会删除已保存的标签。
...
有机会，但不是默认保证。6～8 GB 的兼容显卡默认使用近似段级时间戳；你可以主动选择“实验性精确时间戳”，素刀会先加载 ASR 和 Aligner 做本机显存自检。通过后才保存精确模式；失败时会明确提示改用近似模式，不会静默降级。

兼容且总显存至少 8 GB 的显卡默认提供精确模式。没有兼容 GPU 时，系统内存至少 16 GB 才提供 CPU 近似模式，且不承诺实时速度。显存无法同时容纳 imsearch、ASR 和 Aligner 时，素刀会先完成当前 imsearch 工作并卸载其模型，再按 ASR → Aligner → 恢复 imsearch 的顺序处理；任一单独阶段仍放不下时任务会等待显存并允许取消。
...
检查更新也会比较 imsearch 与 TransNetV2 的本机版本和目标软件版本。组件需要更新时，应用先按顺序自动下载、校验并替换组件，全部成功后才下载、验签并安装软件本体；任一组件失败或取消都会阻止软件安装。安装包下载完成前可以取消，NSIS 安装程序启动后不能再由应用取消。
...
自动模式会连续完成常规步骤，不逐项询问。是否使用外部素材由全局设置和会话开关控制。

询问模式会先集中确认素材、最终文案和背景音乐等关键选择，再继续完成普通编辑。外部素材联网不再单独征求同意，仍会让你确认本次使用的素材；覆盖已有内容前也会先确认。
...
![1](../assets/faq/deepseek-help1.png)
![1](../assets/faq/deepseek-help2.png)
![1](../assets/faq/deepseek-help3.png)
3. 3. 拿到API密钥之后 填入到 API密钥中，你的API密钥一定不要告诉其他人
![1](../assets/faq/deepseek-help4.png)
