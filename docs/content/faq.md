...
“创作助手设置 → 模型配置”左侧的固定供应商由素刀服务端提供，包含供应商名称、API Base URL、模型 ID 选项，以及可选的配置说明简介和链接。固定供应商只能选择服务端提供的模型 ID；有配置说明时，客户端会通过 warning 提示卡展示，并在供应商提供链接时附上完整教程入口。你也可以添加自定义供应商并自行输入模型 ID，但仅支持 OpenAI Compatible 协议。暂时离线时会使用同一远程服务最近一次成功获取的供应商目录；没有缓存时仍可使用已有本地配置。
